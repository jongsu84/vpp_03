...
const _hfHeroKPI=()=>{
  ...
  return `<div class="g5">
    ...
    <div class="card"><div class="ct">실시간 통신 상태 ${window.tip('실시간 통신 상태','RTU·인버터·PCS와의 통신 정상 자원 수 / 전체 자원 수','정상 = ACK 응답 + CRC 일치 + 응답 0.5초 이내','0.5초는 SLA 임계치 — 초과 시 통신 지연 알람 발생 / RTU 1초 폴링 기준')}</div><div class="kv" style="display:flex;align-items:center;gap:8px;font-size:18px"><span class="hb-dot ok"></span>${okC}<span style="font-size:11px;color:var(--semantic-label-alt)">/${all.length}</span></div><div class="kd up">0.5초 주기 ${warnC+errC>0?'· ⚠️ '+(warnC+errC)+'건 이상':'· 전원 정상'}</div></div>
