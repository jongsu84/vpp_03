...
<div class="card"><div class="sh"><div class="st">모델 성능 순위</div><button class="cb p" id="chmp-select-btn" style="font-size:10px" onclick="chmpToggleSelect()">챔피언 변경</button></div>
...
window._chmpRenderStatuses=function(){
  document.querySelectorAll('#chmp-tbody tr').forEach(tr=>{
    ...
    if(window._chmpSelectMode && status!=='champion'){
      cell.innerHTML='<button class="cb n" style="font-size:9px;padding:2px 8px" onclick="chmpPromote(\''+tr.dataset.model+'\')">지정</button>';
    } else {
...
window.chmpToggleSelect=function(){
  ...
  if(btn) btn.textContent=window._chmpSelectMode?'취소':'챔피언 변경';
