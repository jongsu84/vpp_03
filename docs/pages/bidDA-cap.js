...
<div class="g4">
  <div class="card acc"><div class="ct">총 설비용량 ${window.tip('총 설비용량','VPP 그룹의 모든 자원 정격 용량 합','Σ(자원별 정격 kW) ÷ 1000 [MW]','Nameplate Capacity 기준 — 시운전·정비 자원 모두 포함')}</div><div class="kv" id="bdc-kpi-total">11.4<span class="ku">MW</span></div><div class="kd neu" id="bdc-kpi-total-sub">실증 · 자원 13개소</div></div>
  ...
  <div class="card"><div class="ct">고장·정비 제외 ${window.tip('고장·정비 제외 용량','정비/고장으로 입찰에서 제외된 용량','상태 = 정비 OR 고장 인 자원 용량 합계','정비 일정은 발전기관리에서 사전 등록 → 자동 입찰 제외')}</div><div class="kv" style="color:var(--palette-yellow-40)" id="bdc-kpi-maint">0.5<span class="ku">MW</span></div><div class="kd neu" id="bdc-kpi-maint-sub">광양항 4단계 정비</div></div>
  <div class="card"><div class="ct">Ramp-rate 한계 ${window.tip('Ramp-rate 한계','단위 시간당 출력 변동 가능 최대치','Σ(자원별 Ramp Rate) [MW/min]','입찰 시 시간대 간 차이가 이 값을 초과하면 거부됨 — 인버터·PCS 사양에 의해 결정')}</div><div class="kv" id="bdc-kpi-ramp">2.8<span class="ku">MW/min</span></div><div class="kd neu" id="bdc-kpi-ramp-sub">인버터 기준</div></div>
</div>
<div class="g65">
...
// ===== VPP 그룹별 자원 데이터 =====
window._bdcVppData={
  '전체':       {total:11.4, maintenance:0.5, ramp:2.8, count:13, sub:'태양광 5 + 풍력 2 + ESS 2 + 바이오 2 + V2G 2',     maintNote:'광양항 4단계 정비'},
  'VPP-전남권': {total:9.5,  maintenance:0.5, ramp:2.0, count:9,  sub:'태양광 4 + 풍력 1 + 바이오 2 + V2G 2',           maintNote:'광양항 4단계 정비'},
  'VPP-제주권': {total:1.0,  maintenance:0,   ramp:0.4, count:3,  sub:'태양광 1 + ESS 2',                              maintNote:'정비 자원 없음'},
  'VPP-경북권': {total:0.9,  maintenance:0,   ramp:0.4, count:2,  sub:'풍력 2',                                        maintNote:'정비 자원 없음'}
};
// 자원 유형 필터 시 적용 비율 (단순 모델)
window._bdcTypeMul={'all':1.0,'태양광':0.55,'풍력':0.30,'ESS':0.08,'바이오':0.05,'V2G':0.02};

// ===== 입력 상수 + 동적 상태 =====
window._bdcConst={totalCap:11.4, maintenance:0.5}; // 호환성 유지 (Feasible 재계산이 사용)
...
// KPI 4종 동적 갱신 (VPP + 유형 + 안전마진 반영)
window._bdcUpdateKPI=function(){
  var p=window._bdcParams;
  var vpp=document.getElementById('bdc-vpp')?.value||'전체';
  var type=document.getElementById('bdc-type')?.value||'all';
  var data=window._bdcVppData[vpp]||window._bdcVppData['전체'];
  var tMul=window._bdcTypeMul[type]||1.0;
  // 자원 유형 필터 적용
  var total=data.total*tMul;
  var maint=data.maintenance*tMul;
  var ramp=data.ramp*tMul;
  var feasible=(total-maint)*(1-p.margin/100);

  // 호환성: _bdcConst도 동기화 (Feasible 재계산 버튼이 의존)
  window._bdcConst={totalCap:total, maintenance:maint};

  // KPI 1: 총 설비용량
  var elT=document.getElementById('bdc-kpi-total');
  if(elT) elT.innerHTML=total.toFixed(1)+'<span class="ku">MW</span>';
  var elTs=document.getElementById('bdc-kpi-total-sub');
  if(elTs) elTs.textContent=type==='all'?(data.count+'개소 · '+data.sub):'필터: '+type+' ('+vpp+')';
  ...
  // KPI 3: 정비 제외
  var elM=document.getElementById('bdc-kpi-maint');
  if(elM) elM.innerHTML=maint.toFixed(1)+'<span class="ku">MW</span>';
  var elMs=document.getElementById('bdc-kpi-maint-sub');
  if(elMs) elMs.textContent=maint>0?data.maintNote:'정비 자원 없음';

  // KPI 4: Ramp-rate (그룹별 자원 합계 — 파라미터 상한과 다름)
  var elR=document.getElementById('bdc-kpi-ramp');
  if(elR) elR.innerHTML=ramp.toFixed(1)+'<span class="ku">MW/min</span>';
  var elRs=document.getElementById('bdc-kpi-ramp-sub');
  if(elRs) elRs.textContent=type==='all'?(vpp+' 합계 (인버터 기준)'):type+' Ramp 합계';
};
...
window.bidCapApply=function(){ window._bdcUpdateKPI(); window._bdcRender(); };
