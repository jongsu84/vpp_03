...
window.P = window.P || {};
/* ===== 하루전입찰: 가격 전략 — 상속 모델 (Option β) ===== */
/* 정책 우선순위: 자원 OR > 그룹 오버라이드 > 표준 정책 (글로벌) */
...
/* 표준 정책 (글로벌 — 모든 그룹의 기본값) */
window.bdpStandardPolicy=window.bdpStandardPolicy||{
...
/* VPP 그룹 오버라이드 (선택적 — 표준 정책과 다른 경우만 설정) */
/* 비어있는 그룹은 표준 정책 상속 / 일부 자원 유형만 오버라이드 가능 */
window.bdpPolicy=window.bdpPolicy||{
  // 초기에는 모든 그룹이 표준 정책 상속 (오버라이드 없음)
  // 운영자가 그룹 선택 후 [저장] 시 그룹 오버라이드 생성
};

/* 자원별 오버라이드 (특수 케이스) */
window.bdpResourceOverrides=window.bdpResourceOverrides||{};

/* 현재 선택된 VPP 그룹 (필터 상태) — '전체' = 표준 정책 편집 모드 */
window._bdpCurrentVPP='전체';
...
<div class="card mb">
  <div class="sh">
    ...
    <div style="display:flex;gap:6px;align-items:center">
      ...
      <button class="cb p sm" onclick="bdpSavePolicy()" style="font-size:11px">저장</button>
    </div>
  </div>
  ...
  <div style="font-size:11px;color:var(--semantic-label-alt);margin-top:10px;line-height:18px;padding:8px 10px;background:var(--semantic-background-2);border-radius:4px">
    ℹ 입찰가 = <b>max(변동비, min(SMP × 가중치, 상한가))</b> · 변동비 미만 자동 차단 · 정책 우선순위: <b>자원 OR > 그룹 OR > 표준 정책</b><br>
    💡 [전체] 선택 시 표준 정책 편집 (모든 그룹 일괄 배포) · 특정 그룹 선택 시 해당 그룹 오버라이드 (표준 정책과 동일하게 저장 시 오버라이드 자동 제거)
  </div>
</div>
...
<div class="card mb">
  ...
  <div style="font-size:12px;color:var(--semantic-label-alt);margin-top:8px;line-height:18px">※ <b>정책 출처</b>: <span style="color:#0a7">표준 정책</span>(글로벌 기본) / <span style="color:#e80">그룹 OR</span>(VPP 단위 오버라이드) / <span style="color:#d32">자원 OR</span>(개별 오버라이드)</div>
</div>
...
window.bdpRenderContext=function(){
  ...
  if(vpp==='전체'){
    // 전체 모드 — 표준 정책 편집
    if(ttl) ttl.textContent='📋 표준 정책 편집 모드 — 모든 VPP 그룹의 기본값';
    if(sub) sub.textContent='글로벌 표준 정책. 그룹별 오버라이드가 없는 자원에 일괄 적용됩니다. 변경 시 모든 그룹의 미오버라이드 자원에 즉시 반영.';
    if(rgn){ rgn.textContent='표준'; rgn.className='badge ok'; rgn.style.fontSize='11px'; }
    // KPI — 글로벌 평균
    var allRules=Object.values(window.MARKET_RULES);
    var smpAvg=allRules.reduce(function(s,r){return s+r.smpAvg;},0)/allRules.length;
    var smpEl=document.getElementById('bdp-kpi-smp');
    if(smpEl) smpEl.innerHTML=smpAvg.toFixed(0)+'<span class="ku">원/kWh</span>';
    var smpSub=document.getElementById('bdp-kpi-smp-sub');
    if(smpSub) smpSub.textContent='3개 그룹 평균';
    var minEl=document.getElementById('bdp-kpi-min');
    if(minEl) minEl.innerHTML='<span style="font-size:14px">육지 0 / 제주 -500</span>';
    var minSub=document.getElementById('bdp-kpi-min-sub');
    if(minSub) minSub.textContent='그룹별 차등 적용';
    // 변동비/정확도 — 모든 자원 가중평균
    var rows=Array.from(document.querySelectorAll('#bdp-merit-tbody tr[data-vpp]'));
    ...
    rows.forEach(function(tr){
      ...
      var std=window.bdpStandardPolicy[typ];
      if(std){ sumCost+=std.cost*cap; sumNmae+=nm*cap; totalCap+=cap; }
    });
    ...
    if(ceSub) ceSub.textContent='전 자원 가중평균 ('+rows.length+'자원)';
    ...
    if(aeSub) aeSub.textContent='전 자원 평균 ('+rows.length+'자원)';
    return;
  }
  ...
  if(!rule) return;
  var hasOverride=!!(window.bdpPolicy[vpp]);
  if(ttl) ttl.textContent='🏷️ '+vpp+' ('+rule.region+') — '+(hasOverride?'그룹 오버라이드 모드':'표준 정책 상속')+'';
  ...
  // KPI
  var smpEl2=document.getElementById('bdp-kpi-smp');
  if(smpEl2) smpEl2.innerHTML=rule.smpAvg+'<span class="ku">원/kWh</span>';
  var smpSub2=document.getElementById('bdp-kpi-smp-sub');
  if(smpSub2) smpSub2.textContent=rule.region+' 시장';
  var minEl2=document.getElementById('bdp-kpi-min');
  if(minEl2){
    var color=rule.minPrice<0?'color:var(--semantic-negative-normal)':'';
    minEl2.innerHTML='<span style="'+color+'">'+rule.minPrice+'</span><span class="ku">원/kWh</span>';
  }
  var minSub2=document.getElementById('bdp-kpi-min-sub');
  if(minSub2) minSub2.textContent=rule.region==='제주'?'음(-)가격 입찰 가능':'육지 재생입찰';
  // 변동비 평균 (그룹 내 자원 — 그룹 OR 우선, 없으면 표준)
  var rows2=Array.from(document.querySelectorAll('#bdp-merit-tbody tr[data-vpp="'+vpp+'"]'));
  var totalCap2=0,sumCost2=0,sumNmae2=0;
  rows2.forEach(function(tr){
    var cap=parseFloat(tr.dataset.cap)||0;
    var typ=tr.dataset.type;
    var nm=parseFloat(tr.dataset.nmae)||0;
    var groupPol=window.bdpPolicy[vpp]&&window.bdpPolicy[vpp][typ];
    var p=groupPol||window.bdpStandardPolicy[typ];
    if(p){ sumCost2+=p.cost*cap; sumNmae2+=nm*cap; totalCap2+=cap; }
  });
  var avgCost2=totalCap2>0?(sumCost2/totalCap2):0;
  var avgNmae2=totalCap2>0?(sumNmae2/totalCap2):0;
  var ce2=document.getElementById('bdp-kpi-cost');
  if(ce2) ce2.innerHTML=avgCost2.toFixed(1)+'<span class="ku">원/kWh</span>';
  var ceSub2=document.getElementById('bdp-kpi-cost-sub');
  if(ceSub2) ceSub2.textContent=rows2.length+'개 자원 가중평균';
  var ae2=document.getElementById('bdp-kpi-acc');
  if(ae2) ae2.innerHTML=(100-avgNmae2).toFixed(1)+'<span class="ku">%</span>';
  var aeSub2=document.getElementById('bdp-kpi-acc-sub');
  if(aeSub2) aeSub2.textContent='Merit Order 기준 ('+rows2.length+'자원)';
};

// ===== 자원 유형 카드 갱신 (전체=표준 / 그룹=오버라이드) =====
window.bdpRenderTypeCards=function(){
  var vpp=window._bdpCurrentVPP;
  var isAll=(vpp==='전체');
  // 컨텍스트 라벨
  var ctxLbl=document.getElementById('bdp-pol-ctx-label');
  if(ctxLbl) ctxLbl.textContent=isAll?'— 표준 정책 (모든 그룹 기본값)':'— '+vpp+' 그룹 오버라이드';
  // 자원 유형별 카운트 (전체: 모든 그룹 합계 / 그룹: 해당 그룹만)
  var typeCount={};
  var sel=isAll?'#bdp-merit-tbody tr[data-vpp]':'#bdp-merit-tbody tr[data-vpp="'+vpp+'"]';
  ...
  Object.keys(window._bdpTypeMap).forEach(function(type){
    ...
    var groupPol=!isAll&&window.bdpPolicy[vpp]&&window.bdpPolicy[vpp][type];
    ...
    if(card){
      ...
      if(isAll){
        // 전체 모드: 표준 정책 카드 (자원 수만 표시)
        countBadge.textContent='표준 · '+n+'자원';
        countBadge.style.background='var(--semantic-tag-bg-blue,#e8f2ff)';
        countBadge.style.color='var(--semantic-tag-label-blue,#0059ff)';
        card.title='표준 정책 — 모든 그룹의 기본값으로 적용';
      } else if(groupPol){
        // 그룹 모드 + 오버라이드 있음
        countBadge.textContent='그룹 OR · '+n+'자원';
        countBadge.style.background='var(--semantic-tag-bg-yellow,#fff5d6)';
        countBadge.style.color='var(--semantic-tag-label-yellow,#e80)';
        card.title=vpp+'에서 표준 정책을 덮어쓴 오버라이드 정책';
      } else {
        // 그룹 모드 + 오버라이드 없음 (표준 상속)
        countBadge.textContent=n>0?('표준 상속 · '+n+'자원'):'표준 상속 (자원 없음)';
        ...
        card.title='표준 정책에서 상속 — 변경 후 저장 시 그룹 오버라이드 생성';
      }
    }
...
window.bdpUpdatePreview=function(){
  ...
  if(vpp==='전체'){
    var allRules=Object.values(window.MARKET_RULES);
    smp=allRules.reduce(function(s,r){return s+r.smpAvg;},0)/allRules.length;
  } else {
    var rule=window.MARKET_RULES[vpp];
    if(!rule) return;
    smp=rule.smpAvg;
...
window.bdpResolveResource=function(tr){
  ...
  var rule=window.MARKET_RULES[vpp];
  if(!rule) return null;
  // 효과 정책 결정 (그룹 OR 있으면, 없으면 표준)
  var groupPol=window.bdpPolicy[vpp]&&window.bdpPolicy[vpp][type];
  var std=window.bdpStandardPolicy[type];
  var basePol=groupPol||std;
  if(!basePol) return null;
  var baseSrc=groupPol?'그룹 OR':'표준 정책';
  // 자원 OR 우선
  var resOR=window.bdpResourceOverrides[name];
  if(resOR && resOR.enabled){
    return {
      cost:resOR.cost!=null?resOR.cost:basePol.cost,
      w:resOR.w!=null?resOR.w:basePol.w,
      max:resOR.max!=null?resOR.max:basePol.max,
      smp:rule.smpAvg,
      minPrice:rule.minPrice,
      source:'자원 OR'
    };
  }
  return {cost:basePol.cost,w:basePol.w,max:basePol.max,smp:rule.smpAvg,minPrice:rule.minPrice,source:baseSrc};
};

// ===== Merit Order 렌더 (전체: 모든 자원 / 그룹: 해당 그룹만) =====
window.bdpRenderMeritPrices=function(){
  var vpp=window._bdpCurrentVPP;
  var isAll=(vpp==='전체');
  var ctxLbl=document.getElementById('bdp-merit-ctx-label');
  if(ctxLbl) ctxLbl.textContent=isAll?'— 전체 자원 (표준 정책 적용)':'— '+vpp+' 자원';
  var idx=0;
  document.querySelectorAll('#bdp-merit-tbody tr[data-name]').forEach(function(tr){
    var matchVpp=isAll||tr.dataset.vpp===vpp;
    ...
    if(sourceCell){
      // 정책 출처 3단계 색상: 자원 OR(빨강) / 그룹 OR(주황) / 표준 정책(초록)
      var color=r.source==='자원 OR'?'#d32':(r.source==='그룹 OR'?'#e80':'#0a7');
      sourceCell.innerHTML='<span style="color:'+color+';font-weight:600">'+r.source+'</span> · w='+r.w.toFixed(2);
    }
  });
};

// ===== 정책 저장 (전체 = 표준 정책 / 그룹 = 그룹 오버라이드) =====
window.bdpSavePolicy=function(){
  ...
  if(isAll){
    // 표준 정책 저장 — 그룹 오버라이드 없는 자원에 적용
    var grpsWithOverride=Object.keys(window.bdpPolicy).length;
    var allRows=document.querySelectorAll('#bdp-merit-tbody tr[data-name]').length;
    if(!confirm('표준 정책(글로벌)을 저장하시겠습니까?\n\n· 5개 자원 유형의 표준 정책 갱신\n· 그룹 오버라이드 없는 자원에 즉시 적용\n· 그룹 오버라이드가 설정된 그룹('+grpsWithOverride+'개)은 영향 없음\n\n변경 내역은 감사이력에 영구 보관됩니다.')) return;
    window.bdpStandardPolicy=newPol;
    toast('표준 정책 저장 — 그룹 오버라이드 없는 모든 자원에 일괄 적용');
  } else {
    // 그룹 오버라이드 저장 — 표준 정책과 비교해 변경분만 의미 있음
    var rule=window.MARKET_RULES[vpp];
    if(!rule){toast('VPP 그룹 정보 누락','warn');return;}
    // 표준 정책과 다른 항목 카운트
    var diffCount=0;
    Object.keys(newPol).forEach(function(t){
      var std=window.bdpStandardPolicy[t];
      var p=newPol[t];
      if(!std||std.cost!==p.cost||std.w!==p.w||std.max!==p.max) diffCount++;
    });
    if(diffCount===0){
      // 모든 값이 표준과 동일 → 오버라이드 제거 (표준 상속)
      if(!confirm(vpp+' 정책이 표준 정책과 모두 동일합니다.\n\n그룹 오버라이드를 제거하고 표준 정책으로 상속하시겠습니까?\n(권장 — 표준 정책 변경 시 자동 반영)')) return;
      delete window.bdpPolicy[vpp];
      toast(vpp+' 그룹 오버라이드 제거 — 표준 정책 상속');
    } else {
      if(!confirm(vpp+' 그룹 오버라이드를 저장하시겠습니까?\n\n· 표준 정책과 다른 항목: '+diffCount+'/5 자원 유형\n· '+rule.region+' 시장 규정 (하한가 '+rule.minPrice+'원)\n· 표준 정책 변경에도 이 그룹은 그대로 유지됩니다.\n\n변경 내역은 감사이력에 영구 보관됩니다.')) return;
      window.bdpPolicy[vpp]=newPol;
      toast(vpp+' 오버라이드 저장 — '+diffCount+'개 유형 변경');
    }
  }
  window.bdpUpdatePreview();
...
window.bidPriceApply=function(){
  ...
  // VPP 그룹 변경 (전체 + 3개 그룹 모두 유효)
  var validVpp=(vppVal==='전체'||window.MARKET_RULES[vppVal]);
  if(validVpp && window._bdpCurrentVPP!==vppVal){
  ...
  document.querySelectorAll('#bdp-merit-tbody tr[data-name]').forEach(function(tr){
    var matchVpp=(window._bdpCurrentVPP==='전체'||tr.dataset.vpp===window._bdpCurrentVPP);
...
window['I_bidDA-price']=function(){
  ...
  // VPP 셀렉트는 "전체" 옵션을 유지 — 표준 정책 편집 모드용
  var vppSel=document.getElementById('bdp-vpp');
  if(vppSel) vppSel.value=window._bdpCurrentVPP;
